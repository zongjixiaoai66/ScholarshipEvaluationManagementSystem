    ...
    <div class="addEdit-block">
        <el-form
            ...
            <input id="xueshengId" name="xueshengId" type="hidden">
               <el-col :span="12">
                   <el-form-item class="input" v-if="type!='info'"  label="综合评测编号" prop="zonghepingceUuidNumber">
                       <el-input v-model="ruleForm.zonghepingceUuidNumber"
                                 placeholder="综合评测编号" clearable  :readonly="ro.zonghepingceUuidNumber"></el-input>
                   </el-form-item>
                   <div v-else-if="type=='info'">
                       <el-form-item class="input" label="综合评测编号" prop="zonghepingceUuidNumber">
                           <el-input v-model="ruleForm.zonghepingceUuidNumber"
                                     placeholder="综合评测编号" readonly></el-input>
                       </el-form-item>
                   </div>
               </el-col>
                <el-col :span="12">
                    <el-form-item class="select" v-if="type!='info'"  label="学年" prop="xuenianTypes">
                        <el-select v-model="ruleForm.xuenianTypes" :disabled="ro.xuenianTypes" placeholder="请选择学年">
                            <el-option
                                v-for="(item,index) in xuenianTypesOptions"
                    ...
                    <div v-else-if="type=='info'">
                        <el-form-item class="input" label="学年" prop="xuenianValue">
                        <el-input v-model="ruleForm.xuenianValue"
               ...
               <el-col :span="12">
                   <div v-if="type=='info'">
                       <el-form-item class="input" label="成绩计算得分" prop="zonghepingceChengjiDefen">
                           <el-input v-model="ruleForm.zonghepingceChengjiDefen"
                                     placeholder="成绩计算得分" readonly></el-input>
                       </el-form-item>
                   </div>
               </el-col>
               <el-col :span="12">
                   <div v-if="type=='info'">
                       <el-form-item class="input" label="活动计算得分" prop="zonghepingceHuodongDefen">
                           <el-input v-model="ruleForm.zonghepingceHuodongDefen"
                                     placeholder="活动计算得分" readonly></el-input>
                       </el-form-item>
                   </div>
               </el-col>
               <el-col :span="12">
                   <div v-if="type=='info'">
                       <el-form-item class="input" label="最终得分" prop="zonghepingceZuizhongDefen">
                           <el-input v-model="ruleForm.zonghepingceZuizhongDefen"
                                     placeholder="最终得分" readonly></el-input>
                       </el-form-item>
                   </div>
               </el-col>
                <el-col :span="24">
                    <el-form-item v-if="type!='info' && !ro.zonghepingceContent"  label="备注" prop="zonghepingceContent">
                        <editor style="min-width: 200px; max-width: 600px;"
                                v-model="ruleForm.zonghepingceContent"
                                class="editor"
                                action="file/upload">
                        </editor>
                    </el-form-item>
                    <div v-else-if="type=='info'">
                        <el-form-item v-if="ruleForm.zonghepingceContent" label="备注" prop="zonghepingceContent">
                            <span v-html="ruleForm.zonghepingceContent"></span>
                        </el-form-item>
                    </div>
                </el-col>
            </el-row>
    ...
    export default {
        data() {
            return {
                ...
                ro:{
                    xueshengId: false,
                    zonghepingceUuidNumber: true,
                    xuenianTypes: false,
                    xueqiTypes: false,
                    zonghepingceChengjiDefen: false,
                    zonghepingceHuodongDefen: false,
                    zonghepingceZuizhongDefen: false,
                    zonghepingceContent: false,
                    insertTime: false,
                },
                ruleForm: {
                    xueshengId: '',
                    zonghepingceUuidNumber: new Date().getTime(),
                    xuenianTypes: '',
                    xueqiTypes: '',
                    zonghepingceChengjiDefen: '',
                    zonghepingceHuodongDefen: '',
                    zonghepingceZuizhongDefen: '',
                    zonghepingceContent: '',
                    insertTime: '',
                },
                xuenianTypesOptions : [],
                ...
                rules: {
                   ...
                   zonghepingceUuidNumber: [
                              { required: true, message: '综合评测编号不能为空', trigger: 'blur' },
                          ],
                   xuenianTypes: [
                   ...
                   zonghepingceChengjiDefen: [
                              { required: true, message: '成绩计算得分不能为空', trigger: 'blur' },
                   ...
                   zonghepingceHuodongDefen: [
                              { required: true, message: '活动计算得分不能为空', trigger: 'blur' },
                              {  pattern: /^[0-9]{0,6}(\.[0-9]{1,2})?$/,
                                  message: '只允许输入整数6位,小数2位的数字',
                                  trigger: 'blur'
                              }
                   ...
                   insertTime: [
                              { required: true, message: '评测时间不能为空', trigger: 'blur' },
                          ],
                }
            };
        },
        ...
        created() {
            ...
            //获取下拉框信息
                this.$http({
                    url:`dictionary/page?page=1&limit=100&sort=&order=&dicCode=xuenian_types`,
                    method: "get"
                }).then(({ data }) => {
                    if (data && data.code === 0) {
                        this.xuenianTypesOptions = data.data.list;
                    }
                });
                this.$http({
        ...
        methods: {
            ...
            info(id) {
                let _this =this;
                _this.$http({
                    url: `zonghepingce/info/${id}`,
                    method: 'get'
                }).then(({ data }) => {
                    if (data && data.code === 0) {
                        _this.ruleForm = data.data;
                        _this.xueshengChange(data.data.xueshengId)
                        _this.ruleForm.zonghepingceContent = _this.ruleForm.zonghepingceContent.replaceAll("src=\"upload/","src=\""+this.$base.url+"upload/");
                    } else {
            ...
            onSubmit() {
                this.$refs["ruleForm"].validate(valid => {
                    if (valid) {
                        this.ruleForm.zonghepingceContent = this.ruleForm.zonghepingceContent.replaceAll(this.$base.url,"");
                        this.$http({
                            url:`zonghepingce/${!this.ruleForm.id ? "save" : "update"}`,
                            method: "post",
                            data: this.ruleForm
                        }).then(({ data }) => {
                            if (data && data.code === 0) {
                                this.$message({
                                    ...
                                    onClose: () => {
                                        ...
                                        this.parent.zonghepingceCrossAddOrUpdateFlag = false;
                                        this.parent.search();
