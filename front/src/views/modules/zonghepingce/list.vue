        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '学年' : ''">
                        <el-select v-model="searchForm.xuenianTypes" placeholder="请选择学年">
                            <el-option label="=-请选择-=" value=""></el-option>
                            <el-option
                               v-for="(item,index) in xuenianTypesSelectSearch"
                               v-bind:key="index"
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('zonghepingce','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/jiangxuejinpingding/upload/zonghepingceMuBan.xls"
                        >批量导入综合评测数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('zonghepingce','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="zonghepingceUploadSuccess"
                                :on-error="zonghepingceUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('zonghepingce','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入综合评测数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('zonghepingce','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "zonghepingce.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="zonghepingceUuidNumber"
                                   header-align="center"
                                   label="综合评测编号">
                        <template slot-scope="scope">
                            {{scope.row.zonghepingceUuidNumber}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="xuenianTypes"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="zonghepingceChengjiDefen"
                                   header-align="center"
                                   label="成绩计算得分">
                        <template slot-scope="scope">
                            {{scope.row.zonghepingceChengjiDefen}}
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="zonghepingceHuodongDefen"
                                   header-align="center"
                                   label="活动计算得分">
                        <template slot-scope="scope">
                            {{scope.row.zonghepingceHuodongDefen}}
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="zonghepingceZuizhongDefen"
                                   header-align="center"
                                   label="最终得分">
                        <template slot-scope="scope">
                            {{scope.row.zonghepingceZuizhongDefen}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('zonghepingce','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('zonghepingce','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('zonghepingce','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
...
<script>
    ...
    //当前表下拉框搜索条件
              xuenianTypesSelectSearch : [],
              xueqiTypesSelectSearch : [],
            form:{
                id : null,
                xueshengId : null,
                zonghepingceUuidNumber : null,
                xuenianTypes : null,
                xueqiTypes : null,
                zonghepingceChengjiDefen : null,
                zonghepingceHuodongDefen : null,
                zonghepingceZuizhongDefen : null,
                zonghepingceContent : null,
                insertTime : null,
                createTime : null,
            },
            dataList: [],
            ...
            json_fields: {
                //级联表字段
                     ...
                     '账户状态': 'jinyongTypes',
                //本表字段
                     '综合评测编号': "zonghepingceUuidNumber",
                     '学年': "xuenianValue",
                     '学期': "xueqiValue",
                     '成绩计算得分': "zonghepingceChengjiDefen",
                     '活动计算得分': "zonghepingceHuodongDefen",
                     '最终得分': "zonghepingceZuizhongDefen",
                     '评测时间': "insertTime",
            },
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'zonghepingce',//当前表表名,
                        sumColum : 'zonghepingce_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'zonghepingce_name',//分组字符串字段
                        // types : 'zonghepingce_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
                ...
                _this.$nextTick(() => {
                    ...
                    this.$http({
                        url: "barSumTwo",//barCountOne barCountTwo barSumOne barSumTwo
                        method: "get",
                        params: {}
                    }).then(({data}) => {
                        if(data && data.code === 0){
                            let yAxisName = "总得分";//y轴
                            let xAxisName = "学年";//x轴
                            let series = [];//具体数据值
                ...
                //         groupColumn: "zonghepingce_types",
                //// riqi :getMonthFormat(_this.echartsDate),//年月
                //     }
                //     this.$http({
            ...
            getDataList() {
                ...
                if (this.searchForm.xuenianTypes!= '' && this.searchForm.xuenianTypes!= undefined) {
                    params['xuenianTypes'] = this.searchForm.xuenianTypes
                }
                ...
                params['zonghepingceDelete'] = 1// 逻辑删除字段 1 未删除 2 删除


                this.$http({
                    url: "zonghepingce/page",
                    method: "get",
                    params: params
                }).then(({data}) => {
                    if(data && data.code === 0){
                ...
                //填充下拉框选项
                this.$http({
                    url: "dictionary/page?dicCode=xuenian_types&page=1&limit=100",
                    method: "get",
                }).then(({data}) => {
                    if(data && data.code === 0){
                        this.xuenianTypesSelectSearch = data.data.list;
                    }
                });
                //填充下拉框选项
                this.$http({
            ...
            // 导入功能上传文件成功后调用导入方法
            zonghepingceUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "zonghepingce/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
            ...
            wuyong(id) {
                ...
                _this.$confirm(`确定    操作?`, "提示", {confirmButtonText: "确定",cancelButtonText: "取消",type: "warning"
                    ...
                    _this.$http({
                        url: "zonghepingce/update",
                        method: "post",
                        data: {
                            id:id,
//                            zonghepingceTypes:zonghepingceTypes,
                        }
                    }).then(({data}) => {
                        if(data && data.code === 0){
